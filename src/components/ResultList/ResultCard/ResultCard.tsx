...
const ResultCard = ({ result, isFavorite, toggleFavoriteResult }: Props) => {
  ...
  return (
    <li key={id} className="relative flex">
      <Link href={`/${result.slug}`} className="flex w-full flex-col shadow-md">
        ...
        <div className="flex w-full flex-1 flex-col space-y-4 p-4">
          ...
          <dl className="space-y-3 text-sm sm:text-base sm:[&_dd]:text-sm sm:[&_dd]:font-semibold sm:[&_dt]:font-normal sm:[&_img]:row-span-2 sm:[&_img]:mt-1">
